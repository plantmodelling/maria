...
float P_angInitMoyVertPrim=0.7854; /* Angle d'insertion moyen par rapport à la verticale pour les primaires */
float P_angInitETVertPrim=0.5;  /* écart-type de l'angle d'insertion des primaires */

// Caractérisation de l'émission des tardives
float P_ageEmissionTard=12.0; /* âge de commencement de l'émission des racines tardives */
...
int P_sacrifice=1; /* Swith on/off pour le sacrifice*/
...
char P_outputName [80] = "root_image.rsml";  /* Name of the output file*/
...
{ /* Assure l'évolution de la pointe */
  ...
  double volDisponible=sR->volMax[temps];
...
char bid[MAXLINE];
  ...
  fscanf(FPar,"%f",&P_ageEmissionTard);
  fgets(bid,MAXLINE-1,FPar); // reste de la ligne

  fscanf(FPar,"%f",&P_dBaseMaxTard);
  fgets(bid,MAXLINE-1,FPar); // reste de la ligne

  fscanf(FPar,"%f",&P_vitEmissionTard);
  fgets(bid,MAXLINE-1,FPar); // reste de la ligne

  fscanf(FPar,"%f",&P_propDiamTard);
  fgets(bid,MAXLINE-1,FPar); // reste de la ligne

  fscanf(FPar,"%i",&P_nbMaxTard);
  fgets(bid,MAXLINE-1,FPar); // reste de la ligne
  ...
  fscanf(FPar,"%f",&P_probaEffetDiam);
  fgets(bid,MAXLINE-1,FPar); // reste de la ligne
...
// MODIFBEN : Ajout de paramètres pour induire la mort d'une certaine racine à un moment défini.
  fscanf(FPar,"%i",&P_sacrifice); // Switch sacrifice
  fgets(bid,MAXLINE-1,FPar);

  fscanf(FPar,"%i",&P_condemnedRoot); // ID de la racines condamnée
  fgets(bid,MAXLINE-1,FPar);

  fscanf(FPar,"%i",&P_sacrificeTime); // Temps auquel le sacrifice doit être effectué
  fgets(bid,MAXLINE-1,FPar);
...
{   /* Imprime l'entête du fichier contenant les noeuds du système racinaire */
  FSeg=fopen(P_outputName,"w"); //MODIFBEN passage à outputname2, incluant le pas de temps
...
fprintf(FSeg,"    <resolution>30</resolution>\n");
...
fprintf(FSeg,"      <parameter name='P_sacrifice'>");
fprintf(FSeg,"%i",P_sacrifice);
fprintf(FSeg,"</parameter>\n");

fprintf(FSeg,"      <parameter name='P_condemnedRoot'>");
fprintf(FSeg,"%i",P_condemnedRoot);
fprintf(FSeg,"</parameter>\n");

fprintf(FSeg,"      <parameter name='P_sacrificeTime'>");
fprintf(FSeg,"%i",P_sacrificeTime);
fprintf(FSeg,"</parameter>\n");
