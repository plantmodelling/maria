...
public class RootAnalysis {
	...
	static boolean blackRoots, saveImages, saveTips, verbatim, saveTPS;

	// Estimators
	static float area, depth, length, width, comY, comX, Ymid, Xmid, bX, bY;
	static float[] params = new float[71];
    static float[] xCoord;
    static float[] yCoord;
...
//        getWhiteEllipses(currentImage.duplicate());
        ...
        getConvexHull(currentImage.duplicate());
	...
	private void getGeometry(ImagePlus im, ImagePlus skel){
        ...
        an = new Analyzer(skel, Measurements.AREA | Measurements.CENTER_OF_MASS | Measurements.RECT, rt);
        rt.reset();        
        an.measure();
        
        // Area
        length = (float) rt.getValue("Area", 0);
	    ...
	    //Center of Mass
	    params[counter++] = (float) rt.getValue("XM", 0) / width;
	    params[counter++] = (float) rt.getValue("YM", 0) / depth;  
	...
	private void getPixelProfiles(ImagePlus im){
		...
		for(int j = 0; j < 4; j++){
			...
			for(int i = (j+1) * inc; i > j * inc; i = i-2){
				Rectangle roi = new Rectangle(0, i, (int) width, 1);
				im.setRoi(roi);
				Analyzer.setResultsTable(rt);
				rt.reset();
				an = new Analyzer(im, Measurements.AREA | Measurements.AREA_FRACTION, rt);
			    an.measure();
			    tot = (float) (rt.getValue("%Area", 0) * rt.getValue("Area", 0));
				if(tot > 0){
					sum += (float) tot;
					count++;
				}
	...
	private void getConvexHull(ImagePlus im){
	...
	private void printParamCSVHeader(){	
		...
		for(int i = 0; i < 4; i++) toPrint = toPrint.concat(",cross_hori_"+i+"_mean,cross_hori_"+i+"_max");
